  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const response = await axios.post(
        'https://api.cloudinary.com/v1_1/iamraselmolla/auto/upload', formData);

      if (response.data.url) {
        const result = await axios.put(`/api/addresearch?id=${localid}`, {
          fileLink: response.data.url
        });

        if (result.status === 200) {
          toast.success('Research File added successfully');
